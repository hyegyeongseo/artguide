...
| `hand_structure` | hand, anatomy | ◯ VLM 관찰자(`HAND_VLM`) — 관찰=가설 |
...
### 손 영역 관찰 — VLM 관찰자
손(`hand_structure`)은 키포인트 검출이 *그림에선 무력*합니다(MediaPipe는 사진에만 동작 → 스케치에서 0 검출). 그래서 `HAND_AUTO` 키포인트 경로는 기본 OFF이고, 대신 **VLM 관찰자**(`ml/vision.py`, 게이트 `HAND_VLM`)가 손을 *본다*:

- `observe_hand`가 손을 **구조화 JSON**(view 손등/손바닥/옆면 · structure 입체/평면 · foreshortening 손가락 목록)으로 관찰하고, **2회 호출 교차 일관성**이 맞을 때만 신뢰도 `관찰`(아니면 `낮음`으로 기권).
- 결과는 진단에 **measured=False(관찰=가설)** 로 주입됩니다 — "측정=사실"을 오염시키지 않고, 사용자가 손을 명시 요청하면(`from_user`) 가이드 헤드라인으로 올라갑니다.
- 백엔드는 `VLM_BACKEND`로 전환 — **AI Studio**(API 키) 또는 **Vertex**(GCP ADC · global 엔드포인트, 한도·크레딧 유리). 게이트 OFF면 기존 빈 관찰로 폴백(동작 불변).
- 충실도는 `eval_harness/hand_faithfulness.py`로 검증합니다(사람 라벨 대비 view/structure 정확도 · foreshortening P/R · 일관성↔정확성). **구체 관찰 노출은 이 충실도 통과 후에만** 엽니다.
...
> **계측 연속성** — `SUBJECT_MASK`처럼 *측정 가능 영역*을 바꾸는 변경은 정확도 개선이 아니라 **계측 장치 변경**입니다. 성장 비교가 "실력 변화 vs 계측 변화"를 섞지 않도록, 연습 로그에 `instrument_version`(`dx-2026.06`, 마이그레이션 `011_practice_instrument`)을 박아 버전 경계에서 비교를 끊습니다.
...
임베더는 `ml/embed.py`가 `EMBEDDING_MODEL`(env)로 결정하며, 기본값은 **OpenCLIP `ViT-L-14:openai`(= `clip-vit-large-patch14`, 768차원)** 로 **drawe Spring의 CLIP 임베딩 공간과 동일**합니다. 차원은 하드코딩 없이 모델에서 가져와(`embedder.dim`) Qdrant 컬렉션 생성·검증에 사용하므로, 모델만 바꾸면 코드 수정 없이 차원이 따라갑니다.
...
# 축별 충실도 자(harness) — 능력 추가 전 평가셋부터 만든다는 규율
python -m eval_harness.run_value <폴더> --labels <...>           # 명암(value) 축 신호 충실도
python -m eval_harness.hand_faithfulness <폴더> --labels <...>   # 손 관찰 충실도(view/structure·foreshortening P/R·일관성↔정확성)
```
GitHub Actions(`.github/workflows/ci.yml`)가 push/PR마다 **Lint+자산 SSOT drift · 백엔드 커스텀 테스트 · Docker 빌드 스모크**를 실행합니다. 별도 `qdrant-keepalive.yml`이 Qdrant 클라우드를 깨어 있게 유지합니다.
...
```
.
├── .github/workflows/         # ci.yml(lint·자산 drift·백엔드 테스트·도커 스모크) · qdrant-keepalive.yml
...
│   │   ├── ml/                   # embed · scene · pose · normalize · llm · hands · vision(손 VLM 관찰자) · upload_guard
│   │   ├── pipeline/             # router · diagnose · search · coach · feedback · ingest · mask(피사체 마스크) · agent(다음 단계)
...
│   │   ├── eval_harness/         # 축별 충실도 자(run_value · hand_faithfulness · gemini_probe)
