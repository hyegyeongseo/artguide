...
> **이 프로젝트의 핵심 — LLM은 *결정자*가 아닙니다.**
> 진단 · 목표 선정 · 반복 패턴 추적 · 로드맵 진행 판정은 모두 **결정적(non-LLM) 로직**(taxonomy · 성장 상태머신 · roadmap · reference corpus · guardrail · feedback rerank)으로 동작하고, LLM은 **코칭 문장 생성에만** 쓰입니다. 그래서 *그림 생성 AI*나 *Gemini 래퍼*가 아니라 **성장 추적 엔진 + 코칭 인터페이스**입니다.
...
```text
입력 그림  →  관찰 기반 코칭(관찰·효과·실험 + 레퍼런스)  →  성장 로드맵(이번 목표 · 진행 · 다음 단계)
```

<!-- 실제 캡처 3장을 docs/screenshots/ 에 넣고 아래 표의 주석을 해제하세요 (01-upload / 02-guide / 03-roadmap).
| 1. 그림 업로드 | 2. 관찰 기반 코칭 | 3. 성장 로드맵 |
|:---:|:---:|:---:|
| ![업로드 화면](docs/screenshots/01-upload.png) | ![코칭 화면](docs/screenshots/02-guide.png) | ![로드맵 화면](docs/screenshots/03-roadmap.png) |
| 그림 + 메시지 입력 | Observation→Effect→Experiment + 레퍼런스 카드 | 이번 목표 · 진행 · 🎉달성→다음 |
-->
...
업로드(그림 + 메시지)
  ...
  ↓ 진단(taxonomy 14축 · 기하/명도/색/빛 신호 · 손 영역은 VLM 관찰자로 관찰=가설 · 성장 이력으로 보정)
...
FastAPI · Pydantic · OpenCLIP(ViT-L/14 · 768차원 — Spring과 동일 CLIP 공간) · MediaPipe Tasks · **Gemini/Vertex VLM**(손 영역 관찰자 · 선택·게이트) · Qdrant(벡터) · MySQL · MinIO(S3) · React + Vite · Docker Compose.
